...
=pod
					...
					if($entity =~ /^(.+)(\-*[0-9]+)[ ]*,[ ]*(\-*[0-9]+)[ ]*(,|,[ ]*and|and|,[ ]*or|or)[ ]*(\-*[0-9]+)$/) #MMP-2, 7, 9
					{
						...
						insertion($pmid,$gmention_type,$sid,$tmp_entity,$start,$last,$majority_voting_species,$tax_extraction,$tax_location_hash{$sid},$sa_extraction,$entity);
...
=cut		
					insertion($pmid,$gmention_type,$sid,$entity,$start,$last,$majority_voting_species,$tax_extraction,$tax_location_hash{$sid},$sa_extraction,"");
				}
			}
		}	
	}
...
sub insertion
{
	...
	elsif($org_entity =~/^[z]([A-Z].*)$/)
	{
		$clear_entity=$1;
		print output "<$gmention_type pmid='$pmid' sid='$sid' start='$start_site' end='$end_site' tax_id='9606' org='$ORG'>$clear_entity</$gmention_type>\n";
		$mode=1;
	}
	elsif($org_entity =~/^zf([A-Z].*)$/)
	{
		$clear_entity=$1;
		print output "<$gmention_type pmid='$pmid' sid='$sid' start='$start_site' end='$end_site' tax_id='9606' org='$ORG'>$clear_entity</$gmention_type>\n";
		$mode=1;
	...
	if($mode == 0)
	{
		...
		if(!($target eq ""))
		{
			...
			if($Org_numEnt ne "")
			{
				print output "<$gmention_type pmid='$pmid' sid='$sid' start='$start_site' end='$end_site' tax_id='9606' org='$ORG'>$org_entity</$gmention_type>\n";
			}
			else
			{
				print output "<$gmention_type pmid='$pmid' sid='$sid' start='$start_site' end='$end_site' tax_id='9606'>$org_entity</$gmention_type>\n";
			}
		}
	}
	if($mode == 0)
